...
public class CalcActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private double mSecondaccumulator;
    private double total;
    private char mOp;
    Boolean mIsFirstTime;
    Boolean mIsFirstOperation;
    ...
    private void makeOperation() {
        ...
        double actualNumberInDisplay = Double.parseDouble(resultTextView.getText().toString());


        if (mIsFirstOperation) {
            mSecondaccumulator = actualNumberInDisplay;
        } else {
            mAccumulator = mSecondaccumulator;

        }

        switch (mOp) {
            case '+':
                total = mAccumulator + actualNumberInDisplay;
        ...
        Log.d("tag", "makeOperation: actualNumberInDisplay" + actualNumberInDisplay);
        Log.d("tag", "makeOperation: mAccumulator" + mAccumulator);
        Log.d("tag", "makeOperation: mSeccondAccumulator" + mSecondaccumulator);
        Log.d("tag", "makeOperation: total" + total);
    }
    ...
    private void deleteNumber() {
        ...
        if (actualNumber.length() == 1 || actualNumber.length() == 2 && (Integer.parseInt(actualNumber) < 0) || (Double.parseDouble(actualNumber) < 0)) {
            resultTextView.setText("0");

        } else {
            if (actualNumber.charAt(actualNumber.length() - 1) != '.') {
                resultTextView.setText(
                        actualNumber.substring(0, actualNumber.length() - 1)
                );
//                Log.d("calc",actualNumber+"if");
            } else {
                resultTextView.setText(
                        actualNumber.substring(0, actualNumber.length() - 2)
                );
//                Log.d("calc",actualNumber+"else");
            }
